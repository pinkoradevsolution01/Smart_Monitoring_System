...
class CodeRequestService {
  ...
  /// Get activation requests for the developer dashboard.
  /// Pass a status to filter on the server, or leave it null to load all.
  Future<List<Map<String, dynamic>>> getRequests({String? status}) async {
    try {
      ...
      debugPrint('Failed to fetch activation requests: $e');
      return [];
    }
  }

  /// Backwards-compatible helper for callers that only need pending requests.
  ...
  Future<String?> getAvailableActivationCode(String packageName) async {
    try {
      ...
      if (codes.isEmpty) {
        debugPrint('No available activation codes for package: $packageName');
        return null;
      }
  ...
  /// Mark an activation code as assigned.
  Future<bool> markActivationCodeAsUsed(String code) async {
    try {
      ...
      debugPrint('Code marked as assigned');
      return true;
    } catch (e) {
