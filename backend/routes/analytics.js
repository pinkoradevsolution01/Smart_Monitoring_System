...
router.get('/sales', async (req, res) => {
  ...
  try { const [total] = await query(`SELECT COUNT(*) total FROM sales WHERE business_id = ? AND datetime >= ? AND datetime < ?`, [businessId, from, toExclusive]); const rows = await query(`SELECT id, cashier_name, payment_method, status, subtotal, discount, total_amount, item_count, datetime, transaction_type, delivery_status FROM sales WHERE business_id = ? AND datetime >= ? AND datetime < ? ORDER BY datetime DESC LIMIT ? OFFSET ?`, [businessId, from, toExclusive, pageSize, offset]); res.json({ success: true, data: rows, pagination: { page: value, pageSize, total: total.total }, meta: meta(req) }); } catch (error) { console.error('Analytics sales error:', error); res.status(500).json({ success: false, message: 'Failed to load sales analytics.' }); }
});
